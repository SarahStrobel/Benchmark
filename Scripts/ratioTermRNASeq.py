...
# parser.add_argument('-ts', dest='termSeqPath', help='path to directory of Term-Seq 5\' nucleotide frequency files in bedgraph format', required=True)
			...
			if startTerminator > numberOfNucsInGenome[0] and endTerminator < numberOfNucsInGenome[1]:
				...
				for i in range(len(linesTS2)):
					...
					if TSstart > numberOfNucsInGenome[0] and TSstart < numberOfNucsInGenome[1]:
						...
						#novoalign output:
						#unique in b.sub: LB1 (ERX1304415): 2522326, LB2 (ERX1320300): 3385757, LB3 (ERX1320301): 3403991, RNA-Seq (ERX1320302): 7414928
						#unique in e.faec: Ctrol1 (ERR1248401): 538790, Ctrol2 (EERR1248402): 581820, Ctrol3 (ERR1248403): 795081, RNA-Seq (ERR1248404): 3963991
						#unique in l.mono: Ctrol1 (ERR1248436): 1120106, Ctrol2 (ERR1248437): 777793, Ctrol3 (ERR1248438): 483904, RNA-Seq (ERR1248439): 4319194
	...
	for i in range(0,(len(linesTS)), numberOfNucsToSplitInto):
		...
		for i in range(len(linesTS3)):
			...
			if TSstart > numberOfNucsInGenome[0] and TSstart < numberOfNucsInGenome[1]:
				...
				# TScount4 = int(lines3TS3[i].split()[3])
				...
				# minTScount = np.min([TScount, TScount2, TScount3, TScount4])
...
if organism == 'E.faecalis' or organism == 'L.monocytogenes' or organism == 'S.pneumoniae':
	...
	with open(RNIEoutfileTSvsRS, 'w') as RNIEtsVsRs, \
		...
		for list1 in strandMaxTSandMeanRNAWholeGenome:
			if list1[0]-100 > 0 and list1[0]+100 <= lengthGenome:
				if list1[3] == '-':
					RNIEtsVsRs.write(chrom + '\t' + str(list1[0] -100) + '\t' + str(list1[0]+20) + '\t' \
						+ str(list1[0]) + '_'+ str(list1[1]) + '_'+ str(list1[2]) + '_' + str(list1[3]) +'\n') # with coords and strand
				if list1[3] == '+':
					RNIEtsVsRs.write(chrom + '\t' + str(list1[0] - 20) + '\t' + str(list1[0]+100) + '\t' \
						+ str(list1[0]) + '_' + str(list1[1]) + '_'+ str(list1[2]) + '_' +str(list1[3]) +'\n') # with coords and strand
				if list1[3] == 'nostrand':
					RNIEtsVsRs.write(chrom + '\t' + str(list1[0] -100) + '\t' + str(list1[0]+20) + '\t' \
						+ str(list1[0]) + '_' + str(list1[1]) + '_'+ str(list1[2])+ '_' +str(list1[3]) +'\n') # with coords and strand
		...
		for list1 in strandTScountsOverlappingGenes:
			if list1[0]-100 > 0 and list1[0]+100 <= lengthGenome:
				if list1[3] == '-':
					RNIEtsOg.write(chrom + '\t' + str(list1[0] -100) + '\t' + str(list1[0]+20) + '\t' \
						+ str(list1[0]) + '_' + str(list1[1]) + '_'+ str(list1[2])+ '_'  +str(list1[3]) +'\n') # with coords and strand
				if list1[3] == '+':
					RNIEtsOg.write(chrom + '\t' + str(list1[0] - 20) + '\t' + str(list1[0]+100) + '\t' \
						+ str(list1[0]) + '_' + str(list1[1]) + '_'+ str(list1[2])+ '_'  +str(list1[3]) +'\n') # with coords and strand
				if list1[3] == 'nostrand':
					RNIEtsOg.write(chrom + '\t' + str(list1[0] -100) + '\t' + str(list1[0]+20) + '\t' \
						+ str(list1[0]) + '_' + str(list1[1]) + '_'+ str(list1[2])+ '_'  +str(list1[3]) +'\n') # with coords and strand
